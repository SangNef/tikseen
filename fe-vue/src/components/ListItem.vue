...
<template>
  <div v-if="selectedItem" @click="selectedItem = null" class="absolute inset-0 w-full h-full overflow-auto bg-black/80 z-10">
    <div @click.stop class="bg-white max-w-md mx-auto mt-20 rounded-lg overflow-hidden cursor-auto">
      <div class="relative">
        <img :src="selectedItem.image" alt="" class="w-full h-auto" />
        <div class="absolute top-0 left-0 p-2">
          <div class="text-xs px-2 py-1 bg-white rounded-full">{{ selectedItem.age }}세</div>
        </div>
        <div class="absolute top-0 right-0 p-2">
          <div class="text-xs px-2 py-1 bg-white rounded-full">{{ selectedItem.size }}</div>
        </div>
        <div class="absolute bottom-0 right-0 p-2">
          <div class="text-xs px-2 py-1 bg-white rounded-full">{{ selectedItem.height }}cm</div>
        </div>
      </div>
      <div class="p-4">
        ...
        <div class="flex gap-2 mt-3">
          <span v-for="tag in selectedItem.tags" :key="tag" class="text-xs px-2 py-1 bg-black text-white rounded-full">{{ tag }}</span>
        </div>
      </div>
    </div>
  </div>
  ...
  <div class="grid grid-cols-2 gap-4">
    <div v-for="(item, i) in items" :key="i" @click="selectedItem = item" class="border rounded-lg overflow-hidden cursor-pointer">
      <div class="relative">
        <img :src="item.image" alt="" class="w-full h-auto" />
        <div class="absolute top-0 left-0 p-2">
          <div class="text-xs px-2 py-1 bg-white rounded-full">{{ item.age }}세</div>
        </div>
        <div class="absolute top-0 right-0 p-2">
          <div class="text-xs px-2 py-1 bg-white rounded-full">{{ item.size }}</div>
        </div>
        <div class="absolute bottom-0 right-0 p-2">
          <div class="text-xs px-2 py-1 bg-white rounded-full">{{ item.height }}cm</div>
        </div>
      </div>
      <div class="p-2">
        ...
        <div class="flex gap-1 mt-2 flex-wrap">
          <span v-for="tag in item.tags" :key="tag" class="text-xs px-2 py-0.5 bg-black text-white rounded-full">{{ tag }}</span>
        </div>
      </div>
    </div>
  </div>
