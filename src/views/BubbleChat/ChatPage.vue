<template>
  <div class="h-screen">
    ...
    <!-- Container của chat -->
    <div class="flex items-center justify-center p-0 h-screen">
      <div
        :style="[!isMobile ? 'position: fixed; bottom: 0;right: 0' : '']"
        class="flex flex-col bg-white rounded-none shadow-xl w-full max-w-full overflow-hidden z-[999999] h-screen">
        <!-- Header -->
        <div
...
const hasBtnHide = ref(false);

const hasBackAction = ref(false);

const backUrl = ref(null);

const savedUserData = ref({
  lastActiveAt: null,
});
...
onMounted(() => {
  ...
  savedUserData.value = JSON.parse(savedUser);
  // kiểm tra nếu  ko thoát màn hình chat thì liên tục cập nhật lastActiveAt
  setInterval(() => {
    if (isChatOpen.value && hasUser.value) {
  ...
  // kiểm tra nếu thoát màn hình chat thì xóa user
  window.addEventListener('beforeunload', () => {
    localStorage.removeItem('user_livechat');
    hasUser.value = false;
    savedUserData.value = {};
  });

  // Đọc tham số has-btn-hide từ URL
  const urlParams = new URLSearchParams(window.location.search);
  hasBtnHide.value = urlParams.get('has-btn-hide') === 'true';

  backUrl.value = urlParams.get('back-url');

  // Kiểm tra thêm thuộc tính has-btn-hide từ iframe nếu có
  try {
    if (window.frameElement && window?.frameElement?.getAttribute('has-btn-hide') === 'true') {
      hasBtnHide.value = true;
      isChatOpen.value = true;
    } else {
      if (backUrl.value) {
        hasBackAction.value = true;
      }
    }
  } catch (error) {
    console.error('Không thể truy cập thuộc tính của iframe:', error);
  }
});
...
function closePopup() {
  if (hasBackAction.value && backUrl.value && !window?.frameElement) {
    return (window.location.href = backUrl.value);
  }

  isChatOpen.value = false;

  // Thử gửi thông báo về cho parent window nếu nằm trong iframe
  try {
    if (window.parent && window.parent !== window) {
      window.parent.postMessage({ action: 'closeChat' }, '*');
    }
  } catch (error) {
    console.error('Không thể gửi thông báo đến parent window:', error);
  }
}
</script>
