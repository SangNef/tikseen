<template>
    <div class="w-full min-h-screen custom-gradient p-4 pb-20 relative">
        <Header title="ĐÁNH GIÁ DOANH NGHIỆP" titleColor="text-[#d9c8ab]" />
        <img :src="whiteGlow" alt="White Glow" class="absolute top-52 left-1/2 -translate-x-1/2 w-66 h-10" />
        <div class="relative mt-10 w-full rounded-xl shadow-xl rounded-corners-effect p-0.25 z-10">
            <div
                ...
                <div class="w-full px-4 pt-2 flex flex-col">
                    <div class="flex items-center gap-2">
                        <img
                            :src="avt"
                            alt="Avatar"
                            class="w-8 h-8 rounded-full object-cover border-2 border-[#383129]" />
        ...
        <div class="mt-6 flex flex-col gap-4">
            <div
                ...
                :key="index"
                class="flex items-center gap-4 bg-[#2b251f] p-4 rounded-xl shadow-xl border border-[#d9c8ab]">
                <img :src="item.logo" :alt="item.name" class="w-16 h-16 object-contain" />
                <div class="flex-1">
                    <h3 class="text-[#d4cbaf] text-lg font-semibold">{{ item.name }}</h3>
                    <div class="flex items-center text-sm mt-1">
                        <template v-for="n in 5" :key="n">
                            <StarIcon class="w-5 h-5" :class="n <= item.rate ? 'text-yellow-400' : 'text-gray-600'" />
                        </template>
                        <span class="ml-2 text-white text-xs">({{ item.rate }})</span>
                    </div>

                    <div class="w-full bg-gray-700 rounded-full h-2.5 mt-2">
                        <div class="bg-[#b08d57] h-2.5 rounded-full" :style="{ width: item.progress + '%' }"></div>
                    </div>
                </div>
            </div>
        </div>
...
import rate from "@landing/data/rate";
