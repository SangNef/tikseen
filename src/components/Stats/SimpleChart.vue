<template>
  <div>
    <div class="relative h-full w-full">
      <!-- Các cột biểu đồ -->
      <div class="flex items-end justify-between h-52 gap-1" v-if="data.length > 0">
        <div
          ...
          <div
            ...
            class="absolute -top-16 left-1/2 transform -translate-x-1/2 bg-gray-900 text-white text-xs rounded-md py-1 px-2 min-w-max">
      ...
      <div v-else class="h-full w-full flex items-center justify-center">
        <div class="text-gray-400 text-sm">Không có dữ liệu hiển thị</div>
      </div>
      ...
      <div
        ...
        <div v-for="n in 4" :key="n" class="relative w-full border-t border-gray-100 text-xs text-gray-400">
          <span class="absolute -top-2.5 -left-7">
            {{ Math.round(maxValue * (1 - (n - 1) / 4)) }}
          </span>
        </div>
      </div>
    </div>
    ...
    <div v-if="data.length > 0" class="flex justify-between mt-2">
      <div v-for="(item, index) in data" :key="index" class="text-xs text-gray-500 text-center flex-1">
        {{ item.label }}
      </div>
    </div>
  </div>
